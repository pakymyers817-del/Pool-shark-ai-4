...
import { AppState, PlayerTarget } from './types';
    ...
    <div className="min-h-screen flex flex-col bg-slate-900 text-slate-100">
      ...
      <footer className="border-t border-slate-800 py-8 bg-slate-900 text-center">
        ...
        <div className="text-xs text-slate-700 font-mono space-y-1">
          <p>Debug Info:</p>
          <p>VITE_KEY: {import.meta.env.VITE_GEMINI_API_KEY ? 'Present (' + import.meta.env.VITE_GEMINI_API_KEY.substring(0, 5) + '...)' : 'Missing'}</p>
          <p>PROCESS_KEY: {process.env.API_KEY ? 'Present (' + String(process.env.API_KEY).substring(0, 5) + '...)' : 'Missing'}</p>
        </div>
      </footer>
    </div>
  );
